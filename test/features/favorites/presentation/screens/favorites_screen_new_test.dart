...
import 'package:movie_discovery_app/features/favorites/presentation/screens/favorites_screen_new.dart';
...
void main() {
  ...
  testWidgets('shows empty state when no favorites', (tester) async {
    final repo = FakeFavoritesRepository(getter: () async => const Right(<FavoriteMovieEntity>[]));
  ...
  testWidgets('shows error state when load fails', (tester) async {
    final repo = FakeFavoritesRepository(getter: () async => const Left(ServerFailure('Oops')));
  ...
  testWidgets('renders list when favorites available and supports remove', (tester) async {
    ...
    // FavoriteMovieItem exposes an overflow or action; simply verify list exists
